...
export const mutations = {
  /**
   * postの中からuseridで投稿を絞り込むんでstate.myPostListにpush
   */
  getPostByMyUserId(state: any) {
    state.myPostList.push(
      state.post.filter((aPost: any) => aPost.userId === state.loginUser.userId)
    )
    console.dir('mutationでの一覧' + JSON.stringify(state.myPostList))
  },
}
