  ...
  methods: {
    ...
    async getComment() {
      ...
      for (const comment of resComments) {
        const formatDate = moment.utc(comment.commentDate).format('yyyy-MM-DD')
        console.log('format後' + formatDate)
        const agoTime = moment
          .utc(comment.commentDate, 'yyyy-MM-DD HH:mm:ss')
          .fromNow()
