  ...
  methods: {
    ...
    async getComment() {
      ...
      for (const comment of resComments) {
        const newDate = new Date(comment.commentDate)
        const formatDate = comment.commentDate.format('YYYY/MM/dd HH:mm:ss')
        console.log('日付フォーマット確認' + formatDate)
        const commentInfo: commentInfo = {
          comment: comment.comment,
          commentDate: formatDate.fromnow(),
      ...
      // format(comment.commentDate, 'YYYY-MM-dd HH:ss:mm')
    },
  },
})
</script>
