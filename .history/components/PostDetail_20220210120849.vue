...
export default Vue.extend({
  ...
  methods: {
    ...
    async getPostDetail() {
      ...
      // 自分がいいねしているかのチェック
      const loginUserName =this.$store.getters['user/getLoginUserName']
      const resultName = this.currentPostDetail.likes.filter(
        (userName) => userName === this.$store.getters['user/getLoginUserName']
      )
      if (resultName === this.$store.getters['user/getLoginUserName']) {
        this.likesFlag = true
      }else if()
    },
